...
#        seq = [(self._DT_standardizer(i[0]), self.all_variables.index(i[1]))\
#            for i in seq if i[1] in self.all_variables]
#        grp_seq = {}
#        for t, c in seq:
...
#            d.append(c)
        ...
        X = [random.sample(line, max_sentence_length)\
                if len(line) >= max_sentence_length\
                    else line + [self.variable_size] * (max_sentence_length -\
                        len(line)) for line in X]
        ...
        if T.shape[0] >= max_sequence_length:
            ...
            padding_values = np.array(
                [[self.variable_size] * max_sentence_length] *\
                    short_seq_length)
            X = np.concatenate((padding_values, X), 0)
