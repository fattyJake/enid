...
class MultiHeadAttention(object):
    ...
    def masking(input_, queries=None, keys=None, type_=None):
        ...
        if type_ in ("k", "key", "keys"):
            ...
            masks = tf.tile(
                masks, [
                    1, tf.shape(queries)[1], 1])  # (N, T_q, T_k)
    ...
    def multi_head_attention_fn(self, scope="multi_head_attention"):
        ...
        with tf.variable_scope(scope):
            ...
            Q_projected = self._linear(self.Q, self.d_model, scope="Q")
            # [batch,sequence_length,d_model]
            K_s_projected = self._linear(self.K_s, self.d_model, scope="K")
            # [batch,sequence_length,d_model]
            V_s_projected = self._linear(self.V_s, self.d_model, scope="V")
            ...
            # 4. linear projection
            # [batch,sequence_length,d_model]
            output = self._linear(dot_product, self.d_model, scope="output")
    ...
    def scaled_dot_product_attention_batch(
        ...
        with tf.variable_scope(scope):
            # 1. split Q,K,V
            # [batch*h,sequence_length,d_k]
            Q_heads = tf.concat(tf.split(Q, self.h, axis=2), axis=0)
            # [batch*h,sequence_length,d_k]
            K_heads = tf.concat(tf.split(K_s, self.h, axis=2), axis=0)
            # [batch*h,sequence_length,d_k]
            V_heads = tf.concat(tf.split(V_s, self.h, axis=2), axis=0)
            ...
            # [batch*h,sequence_length,sequence_length]
            weights = tf.nn.dropout(weights, self.dropout_keep_prob)
...
class LayerNormResidualConnection(object):
    ...
    def layer_normalization(self, x, scope="layer_normalization"):
        ...
        with tf.variable_scope(scope + "_" + str(self.layer_index)):
            # 1. normalize input by using  mean and variance according to last
            # dimension
            ...
            variance = tf.reduce_mean(
                tf.square(
                    x - mean),
                axis=-1,
                keep_dims=True)  # [batch_size,sequence_length,1]
            ...
            scale = tf.get_variable(
                "layer_norm_scale",
                [filter_],
                initializer=tf.ones_initializer)  # [filter]
            bias = tf.get_variable(
                "layer_norm_bias",
                [filter_],
                initializer=tf.zeros_initializer)  # [filter]
            # [batch_size,sequence_length,d_model]
...
class Encoder(object):
    ...
    def encoder_single_layer(self, Q, K_s, layer_index):
        ...
        multi_head_attention_output = \
            self.sub_layer_layer_norm_residual_connection(
                K_s,
                multi_head_attention_output,
                layer_index,
                "mhatt_resid",
    ...
    def sub_layer_multi_head_attention(
        ...
        """
        with tf.variable_scope("encoder_sub_layer_multi_head_attention_" + \
            str(layer_index)):
    ...
    def sub_layer_layer_norm_residual_connection(self,
        ...
        :return:
        """
        with tf.variable_scope("encoder_sub_layer_norm_residual_" + \
            str(layer_index) + '_' + str(scope)):
