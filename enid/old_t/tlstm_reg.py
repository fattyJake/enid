...
class T_LSTM(object):
    ...
    def __init__(
        ...
        self.input_y = tf.placeholder(
            tf.float32, [None, 1], name="input_y"
        )  # y [instance_size, 1]
        ...
        with tf.name_scope("weight"):
            ...
            self.W_final = self._init_weights(
                self.hidden_size, 1, name="Output_Layer_weight", reg=None
        ...
        self.loss_val = tf.identity(
            self._loss(self.l2_reg_lambda), name="loss"
        )
    ...
    def _init_bias(self, output_dim, name):
        return tf.get_variable(
            name, shape=[output_dim], initializer=tf.constant_initializer(0.1)
        )
...
def test_rnn(model_path, step=None, just_graph=False, **kwargs):
    ...
    **kwargs: dict, optional
    ...
    cost_norm = pickle.load(
        open(
            os.path.join(
                os.path.dirname(__file__), "pickle_files", "cost_norm_q"
            ),
            "rb",
        )
    )
    ...
    if just_graph:
        ...
        output = np.empty((0, 1))
        for start, end in zip(
            range(0, number_examples, 128), range(128, number_examples, 128)
        ):
            feed_dict = {
