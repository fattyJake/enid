...
class T_HAN(object):
    ...
    filter_sizes : list of int
        sizes of each CNN inception filters, e.g. [3, 4, 5]

    num_filters : int
        number of each filter
    ...
    >>> model_1 = T_HAN('train', max_sequence_length=50, max_sentence_length=20,
            hidden_size=128, num_classes=2, filter_sizes=[3,4,5], num_filters=128,
            pretrain_embedding=emb, learning_rate=0.05, decay_steps=5000,
            decay_rate=0.9, dropout_keep_prob=0.8, l2_reg_lambda=0.0,
            objective='ce')
    ...
    def __init__(self, mode, **kwargs):
        ...
        assert mode in [
            "train",
            "deploy",
        ], f'AttributeError: mode only acccept "train" or "deploy", got {mode} instead.'
        self.mode = mode
        ...
        if self.mode == "train":
            ...
            with self.graph.as_default():
                ...
                for i, filter_size in enumerate(self.filter_sizes):
                    with tf.name_scope("conv-maxpool-%s" % filter_size):
                        ...
                        W = tf.Variable(
                            ...
                            name="conv-W-%s" % filter_size,
                        )
                        b = tf.Variable(
                            tf.constant(0.1, shape=[self.num_filters]),
                            name="conv-b-%s" % filter_size,
                        )
                        conv = tf.nn.conv2d(
                            self.input,
                            W,
                        ...
                        pooled = tf.nn.max_pool(
                            ...
                            ksize=[
                                1,
                                self.max_sentence_length - filter_size + 1,
                                1,
                                1,
                            ],
                            strides=[1, 1, 1, 1],
                        ...
                        pooled_outputs.append(
                            tf.layers.batch_normalization(pooled)
                        )

                # Combine all the pooled features
                self.h_pool = tf.squeeze(tf.concat(pooled_outputs, 3))
                self.num_filters_total = self.num_filters * len(
                    self.filter_sizes
                )
                ...
                )  # shape:[batch_size,num_sentences,num_filters_total]
