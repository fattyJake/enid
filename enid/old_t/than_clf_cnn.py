...
class T_HAN(object):
    ...
    def __init__(self, mode, **kwargs):
        ...
        assert mode in [
            'train', 'deploy'], f'AttributeError: mode only acccept "train" or "deploy", got {mode} instead.'
        self.mode = mode
        ...
        if self.mode == 'train':
            ...
            with self.graph.as_default():
                ...
                self.input_x = tf.placeholder(
                    ...
                    name="input_x")  # X [instance_size, num_bucket, sentence_length]
                self.input_t = tf.placeholder(
                    tf.int32, [
                        None, self.max_sequence_length], name="input_t")  # T [instance_size, num_bucket]
                self.input_y = tf.placeholder(
                    tf.int32, [
                        None, self.num_classes], name="input_y")  # y [instance_size, num_classes]

                with tf.name_scope("embedding"):
                    self.emb_size = self.pretrain_embedding.shape[1]
                    embedding_matrix = tf.concat(
                        [self.pretrain_embedding, tf.zeros((1, self.emb_size))], axis=0)
                    self.Embedding = tf.Variable(
                        embedding_matrix, trainable=True, dtype=tf.float32, name='embedding')
                ...
                self.input = tf.reshape(
                    self.input, shape=[-1, self.max_sentence_length, self.emb_size])
                ...
                for i, filter_size in enumerate(self.filter_sizes):
                    with tf.name_scope("conv-maxpool-%s" % filter_size):
                        ...
                        b = tf.Variable(
                            ...
                            filter_size)
                        conv = tf.nn.conv2d(
                            self.input, W, strides=[
                                1, 1, 1, 1], padding="VALID", name="conv")
                        # Apply nonlinearity
                        h = tf.nn.relu(
                            tf.nn.bias_add(
                                conv, b), name="conv-relu-%s" %
                            filter_size)
                ...
                self.num_filters_total = self.num_filters * \
                ...
                sentence_representation = tf.reshape(
                    ...
                    shape=[
                        -1,
                        self.max_sequence_length,
                        self.num_filters_total])  # shape:[batch_size,num_sentences,num_filters_total]
                ...
                self.hidden_state_sentence, _ = tf.nn.dynamic_rnn(
                    self.tlstm_cell, concat_input, dtype=tf.float32, time_major=False)
        ...
        if self.mode == 'deploy':
            ...
            if self.step:
                saver = tf.train.import_meta_graph(os.path.join(
                    self.model_path.rstrip('/'), 'model-' + str(self.step) + '.meta'))
    ...
    def _attention_sentence_level(self, hidden_state_sentence):
        ...
        with tf.name_scope("sentence_level_attention"):
            self.W_w_attention_sentence = tf.get_variable(
                "W_w_attention_sentence", shape=[
                    self.hidden_size, self.hidden_size], initializer=self.initializer)
            ...
            self.context_vecotor_sentence = tf.get_variable(
                "what_is_the_informative_sentence", shape=[
                    self.hidden_size], initializer=tf.random_normal_initializer(
                    stddev=0.1))
        ...
        hidden_representation = tf.reshape(
            hidden_representation, shape=[-1, self.max_sequence_length, self.hidden_size])
