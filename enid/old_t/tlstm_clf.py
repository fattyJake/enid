...
class T_LSTM(object):
    ...
    def _init_bias(self, output_dim, name):
        return tf.get_variable(
            name, shape=[output_dim], initializer=tf.constant_initializer(0.0)
        )
        ...
        for epoch in range(curr_epoch, num_epochs):
            ...
            for start, end in zip(
                ...
                curr_loss, _, merged_sum = sess.run(
                    [model.loss_val, model.train_op, model.merged_sum],
                    feed_dict,
                )
                writer.add_summary(
                    merged_sum, global_step=sess.run(model.global_step)
                )
                counter = counter + 1
    ...
    **kwargs: dict, optional
    ...
    if just_graph:
        ...
        if number_examples < 128:
            y_probs = sess.run(
                y_pred, {x: kwargs["x_test"], t: kwargs["t_test"]}
            )[:, 0]
        else:
            y_probs = np.empty((0))
            for start, end in zip(
            ...
            feed_dict = {x: kwargs["x_test"][end:], t: kwargs["t_test"][end:]}
            probs = sess.run(y_pred, feed_dict)[:, 0]
            y_probs = np.concatenate([y_probs, probs])
        sess.close()
        return y_probs
...
def interpret(model_path, step, t_, x_, label, mode="cd", length=50):
    ...
    if mode == "cd":
        code_desc = pickle.load(
            open(
                os.path.join(
                    os.path.dirname(__file__), "pickle_files", "codes"
                ),
                "rb",
            )
        )
    if mode == "more2":
        code_desc = pickle.load(
            open(
                os.path.join(
                    os.path.dirname(__file__), "pickle_files", "codes_more2"
                ),
                "rb",
            )
        )
    code_desc = {
        c: str(d) if len(str(d)) <= 50 else str(d)[:50] + "..."
...
def _do_eval(sess, model, eval_x, eval_t, eval_y, batch_size):
    ...
    eval_acc = sess.run(
        model.auc, {model.input_y: eval_y, model.probs: eval_probs}
    )
