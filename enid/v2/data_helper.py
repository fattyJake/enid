...
#            d.append(c)
        seq = {self._DT_standardizer(t): [self.all_variables.index(
            i) for i in c if i in self.all_variables] for t, c in seq.items()}
        ...
        X = [random.sample(line, max_sentence_length) if len(line) >= max_sentence_length else line + [
            self.variable_size] * (max_sentence_length - len(line)) for line in X]
    ...
    def _DT_standardizer(self, dt):
        ...
        if isinstance(dt, str):
            for i, fmt in enumerate(('%Y-%m-%d', '%m/%d/%Y', '%Y-%m-%dT%H:%M:%SZ', '%Y-%m-%dT%H:%M:%S', '%m/%d/%Y %H:%M',
                                     '%m/%d/%y', '%m/%d/%y %H:%M:%S', '%m/%d/%Y %H:%M:%S', '%m-%d-%Y', '%m-%d-%Y %H:%M')):
                try:
