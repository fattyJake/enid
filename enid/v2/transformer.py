...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        super(
            MultiHeadAttention,
            self).__init__(
            name=f'MultiHeadAttention_{layer_index}',
            **kwargs)
        ...
        self.Q_proj_layer = tf.keras.layers.Dense(
            self.d_model, activation='relu', kernel_initializer='he_normal', name="Q")
        self.K_proj_layer = tf.keras.layers.Dense(
            self.d_model, activation='relu', kernel_initializer='he_normal', name="K")
        self.V_proj_layer = tf.keras.layers.Dense(
            self.d_model, activation='relu', kernel_initializer='he_normal', name="V")
    ...
    def masking(input_, queries=None, keys=None, type_=None):
        ...
        if type_ in ("k", "key", "keys"):
            ...
            masks = tf.sign(
                tf.reduce_sum(
                    input_tensor=tf.abs(keys),
                    axis=-1))  # (N, T_k)
            masks = tf.expand_dims(masks, 1)  # (N, 1, T_k)
            masks = tf.tile(
                masks, [
                    1, tf.shape(
                        input=queries)[1], 1])  # (N, T_q, T_k)
            ...
            masks = tf.sign(
                tf.reduce_sum(
                    input_tensor=tf.abs(queries),
                    axis=-1))  # (N, T_q)
            masks = tf.expand_dims(masks, -1)  # (N, T_q, 1)
            masks = tf.tile(
                masks, [
                    1, 1, tf.shape(
                        input=keys)[1]])  # (N, T_q, T_k)
            # Apply masks to inputs
    ...
    def call(self, inputs):
        ...
        Q_projected = self.Q_proj_layer(Q)
        # [batch,sequence_length,d_model]
        K_s_projected = self.K_proj_layer(K_s)
        # [batch,sequence_length,d_model]
        V_s_projected = self.V_proj_layer(V_s)

        # 2. scaled dot product attention for each projected version of Q,K,V
        dot_product = self.scaled_dot_product_attention_batch(
            Q_projected, K_s_projected, V_s_projected)  # [batch*h,sequence_length,d_k]
    ...
    def scaled_dot_product_attention_batch(
        ...
        # [batch*h,sequence_length,sequence_length]
        weights = tf.nn.dropout(weights, self.dropout_prob)
...
class FeedFoward(tf.keras.layers.Layer):
    ...
    def __init__(self, layer_index, d_model, d_ff, **kwargs):
        ...
        super(
            FeedFoward,
            self).__init__(
            name=f'FeedFoward_{layer_index}',
            **kwargs)
...
class LayerNormResidualConnection(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        super(
            ...
            self).__init__(
            name=f'LayerNormResidualConnection_{mode}_{layer_index}',
            **kwargs)
    ...
    def call(self, inputs):
        ...
        if self.use_residual_conn:
            ...
            x_layer_norm = self.batch_norm_layer(x_residual)
        else:
...
class Encoder(tf.keras.layers.Layer):
        ...
        self.V_s = [
            self.add_weight(
                ...
                shape=[
                    ...
                    self.d_model],
                initializer='he_normal') for layer_index in range(
                self.num_layer)]
        self.encode_stack = [
            {
                ...
                "FF_Resid": LayerNormResidualConnection(
                    ...
                    'FF_Resid',
                    dropout_prob=dropout_prob,
                    use_residual_conn=use_residual_conn)} for layer_index in range(
                self.num_layer)]
    ...
    def encoder_single_layer(self, Q, K_s, layer_index):
        ...
        multi_head_attention_output = self.encode_stack[layer_index]['MultiHeadAttention'](
            [Q, K_s, self.V_s[layer_index]])

        # 1.2 use LayerNorm(x+Sublayer(x)). all dimension=512.
        # [batch_size,sequence_length,d_model]
        multi_head_attention_output = self.encode_stack[layer_index]['MHA_Resid'](
