...
class Attention(tf.keras.layers.Layer):
    ...
    def call(self, hidden_state):
        ...
        token_hidden_representation = tf.reshape(
            token_hidden_representation, shape=[-1, self.sequence_length, self.output_dim])
        ...
        token_p_attention = tf.nn.softmax(
            token_attention_logits,
            name='token_attention')  # [batch_size*num_sentences,sentence_length]
        # [batch_size*num_sentences,sentence_length,1]
        token_p_attention_expanded = tf.expand_dims(token_p_attention, axis=2)
        # [batch_size*num_sentences,sentence_length, d_model]
        representation = tf.multiply(token_p_attention_expanded, hidden_state)
        representation = tf.reduce_sum(
            input_tensor=representation,
