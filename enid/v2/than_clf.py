...
def train_model(model, config, t_train, x_train, y_train, num_epochs: int = 5, model_path: str = 'model',
    ...
    >>> model, config = build_model(2, 30, 40)
    >>> train_model(model, config, t_train, x_train, y_train, num_epoch=2)
    Epoch 1/2
...
class T_HAN(tf.keras.Model):
    ...
    def __init__(self, num_classes: int, pretrain_embedding, max_sequence_length: int, max_sentence_length: int,
        ...
        self.vocab_size, self.emb_size = self.pretrain_embedding.shape
        self.Embedding = tf.Variable(tf.concat([self.pretrain_embedding, tf.zeros((1, self.emb_size))], axis=0), dtype=tf.float32, trainable=True, name='embedding')
    ...
    def call(self, inputs, training=False):
        ...
        inputs = tf.nn.embedding_lookup(self.Embedding, input_x) # [batch_size, num_bucket, sentence_length, embedding_size]
