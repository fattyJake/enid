...
class T_HAN(object):
    ...
    Uses an embedding layer, followed by a token-level bi-GRU with attention, a
    sentence-level time-aware-lstm with attention and sofrmax layer
    ...
    initializer: tf tensor initializer object, optional
        (default: tf.orthogonal_initializer())
    ...
    def __init__(self, mode, **kwargs):
        ...
        assert mode in ['train', 'deploy'], 'AttributeError: mode only '\
            f'acccept "train" or "deploy", got {mode} instead.'
        self.mode = mode
        ...
        if self.mode == 'train':
            ...
            with self.graph.as_default():
                ...
                self.input_t = tf.placeholder(
                    tf.int32, [
                        None, self.max_sequence_length], name="input_t")
                ...
                self.input = tf.reshape(
                    self.input, shape=[-1, self.max_sentence_length,
                        self.emb_size])
                ...
                self.hidden_state = tf.concat(self.hidden_state, axis=2)
                ...
                assert self.objective in ['ce', 'auc'], 'AttributeError: '\
                    'objective only acccept "ce" or "auc", got {}'.format(
                        self.objective)
        ...
        if self.mode == 'deploy':
            ...
            if self.step:
                ...
                saver = tf.train.import_meta_graph(os.path.join(
                    self.model_path.rstrip('/'), 'model.meta'))
                saver.restore(
                    self.sess, tf.train.latest_checkpoint(
                        self.model_path))
            self.graph = tf.get_default_graph()
            ...
            self.max_sequence_length, self.max_sentence_length = int(
                self.max_sequence_length), int(self.max_sentence_length)
    ...
    def train(
        ...
        assert self.mode == "train", 'ModeError: train module need to '\
            'initialize T_HAN.mode as "train".'
        training_size = y_train.shape[0]

        dev_sample_index = -1 * \
            int(dev_sample_percentage * float(training_size))
        ...
        for epoch in range(curr_epoch, num_epochs):
            ...
            for start, end in zip(
                ...
                writer_train.add_summary(
                    merged_sum, global_step=self.sess.run(
                        self.global_step))
                counter = counter + 1
    ...
    def deploy(self, t_test, x_test):
        ...
        if number_examples < 128:
            y_probs = self.sess.run(
                self.probs, {
                    self.input_x: x_test, self.input_t: t_test})[
                :, 0]
        else:
            y_probs = np.empty((0))
            for start, end in zip(
    ...
    def _attention_token_level(self, hidden_state):
        ...
        token_hidden_representation = tf.nn.tanh(
            tf.matmul(
                token_hidden_state_2,
                self.W_w_attention_token) +
            self.W_b_attention_token)
        token_hidden_representation = tf.reshape(
            token_hidden_representation, shape=[-1, self.max_sentence_length,
            self.hidden_size * 2])
        ...
        with tf.name_scope("loss"):
            ...
            l2_losses = tf.add_n([tf.nn.l2_loss(
                v) for v in tf.trainable_variables()\
                    if 'bias' not in v.name]) * l2_reg_lambda
    ...
    def _do_eval(self, eval_x, eval_t, eval_y, batch_size, writer_val):
        ...
        for start, end in zip(
            ...
            eval_loss, eval_probs, eval_counter = eval_loss + \
                curr_eval_loss, np.concatenate([eval_probs, curr_probs]), \
                    eval_counter + 1
        ...
        eval_acc = self.sess.run(
            self.auc, {
                self.input_y: eval_y, self.probs: eval_probs})
