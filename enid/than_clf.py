...
class T_HAN(object):
    ...
    def __init__(self, mode, **kwargs):
        ...
        Uses an embedding layer, followed by a token-level transformer with
        attention, a sentence-level time-aware-lstm with attention and sofrmax
        layer
        ...
        l2_reg_lambda: float, optional (default: .0)
            L2 regularization lambda for fully-connected layer to prevent
        ...
        if self.mode == 'train':
            ...
            self.pretrain_embedding = pickle.load(
                open(
                    os.path.abspath(
                        os.path.join(
            ...
            with self.graph.as_default():
                ...
                self.input_y = tf.placeholder(
                    tf.int32, [
                        self.batch_size, self.num_classes], name="input_y")
                ...
                self.input = tf.reshape(
                    ...
                    shape=[
                        ...
                        self.max_sentence_length,
                        self.emb_size])
                self.input = tf.multiply(self.input, tf.sqrt(
                ...
                sentence_representation = tf.reshape(
                    ...
                    shape=[
                        -1,
                        self.max_sequence_length,
                        self.d_model])  # shape:[batch_size,sequence_lenth,d_model]
                ...
                scan_time = tf.reshape(
                    self.input_t, [
                        tf.shape(
                            ...
                            self.input_t)[1], 1])
                # [batch_size x seq_length x hidden_size+1]
                ...
                self.attention_sum = tf.summary.histogram(
                    "attentions", self.instance_representation)
        ...
        try:
            for epoch in range(curr_epoch, num_epochs):
                ...
                for start, end in zip(
                    range(
                        0, training_size, self.batch_size), range(
                        self.batch_size, training_size, self.batch_size)):
                    ...
                    curr_loss, _, merged_sum = self.sess.run(
                        [self.loss_val, self.train_op, self.merged_sum],
                        feed_dict)
    ...
    def deploy(self, t_test, x_test):
        ...
        if fake_samples > number_examples:
            ...
            t_test, x_test = np.concatenate(
                [t_test] * sup_rec, axis=0), np.concatenate([x_test] * sup_rec,
                axis=0)
            t_test, x_test = t_test[:self.batch_size], x_test[:self.batch_size]

        y_probs = np.empty((0))
        for start, end in zip(range(0, number_examples +
                                    fake_samples +
                                    1, self.batch_size), range(self.batch_size,
    ...
    def _attention_token_level(self, hidden_state):
        ...
        with tf.name_scope("token_level_attention"):
            self.W_w_attention_token = tf.get_variable(
                "W_w_attention_token", shape=[
                    self.d_model, self.d_model], initializer=self.initializer)
            self.W_b_attention_token = tf.get_variable(
                "W_b_attention_token", shape=[self.d_model])
            self.context_vecotor_token = tf.get_variable(
                "what_is_the_informative_token", shape=[
                    self.d_model], initializer=self.initializer)

        token_hidden_state_2 = tf.reshape(
            hidden_state, shape=[-1, self.d_model])
        ...
        sentence_representation = tf.reduce_sum(
            sentence_representation,
            axis=1)  # [batch_size*num_sentences, d_model]
    ...
    def _attention_sentence_level(self, hidden_state_sentence):
        ...
        hidden_state_context_similiarity = tf.multiply(
            hidden_representation, self.context_vecotor_sentence)
        ...
        instance_representation = tf.multiply(
            p_attention_expanded, hidden_state_sentence)
    ...
    def _do_eval(self, eval_x, eval_t, eval_y, writer_val):
        ...
        eval_x, eval_t, eval_y = eval_x[:number_examples - fake_samples], \
            eval_t[:number_examples - fake_samples], \
                eval_y[:number_examples - fake_samples]
        number_examples = number_examples - fake_samples

        eval_loss, eval_counter = 0.0, 0
        eval_probs = np.empty((0, self.num_classes))
        for start, end in zip(range(0, number_examples + 1, self.batch_size),
            range(self.batch_size, number_examples + 1, self.batch_size)):
        ...
        eval_acc = roc_auc_score(eval_y[:, 0], eval_probs[:, 0])
