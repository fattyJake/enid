...
class Claim2Vec(object):
    ...
    attached with  date but no timestamp, the model assume all the claims
    within a day are related
    ...
    def __init__(
        ...
        with self.graph.as_default():
            ...
            # embeddings.
            self.norm = tf.sqrt(
                tf.reduce_sum(tf.square(self.embeddings), 1, keepdims=True)
            )
    ...
    def train(self, data, num_epoch, log_dir, evaluate_every=2000):
        ...
        while sg.get_epoch() < num_epoch:
            ...
            _, summary, loss_val = session.run(
                [self.train_op, self.merged, self.loss], feed_dict=feed_dict
            )
            ...
            if step % (evaluate_every * 10) == 0:
                ...
                sim = self.similarity.eval(
                    feed_dict={self.valid_dataset: valid_examples},
                    session=session,
                )
        ...
        pickle.dump(
            final_embeddings,
            open(os.path.join(log_dir, "embeddings.pkl"), "wb"),
        )
