...
###############################################################################
# Module:      Interpreter
# Description: Visualization module to explain Time-Aware Hierarchical
#              Attention Model
# Authors:     Yage Wang
# Created:     5.1.2019
###############################################################################
...
def monitor_interpreter(
    ...
    for i in range(most_recent)[::-1]:
        t, x = vec({dates[idx]: data[dates[idx]] for idx in range(max(0, len(
            data) - i - most_recent), len(data) - i)},
    ...
    variables = pickle.load(
        open(
            os.path.join(
                ...
                'codes'),
            'rb'))
    data_to_show = {k: [f"{c}  {str(variables.get(c, ''))}" for c in v\
        if re.search(r'^(ICD|GPI|CPT|HCPCS)', c)]\
            for k, v in data_to_show.items()}
    dates = list(data_to_show.keys())
    names = ['<h3>' +
             '<br>'.join([c if len(c) < 100 else c[:97] +
                          '...' for c in v]) +
             '</h3>' for v in data_to_show.values()]
    ...
    with plt.style.context('ggplot'):
        ...
        ax.plot((mdates.date2num(start) -
                 ext, mdates.date2num(stop) +
                 ext), (0, 0), 'k', alpha=.5)
        ax.plot(
...
def attention_interpreter(
    ...
    tokens, x_, sentences = tokens[-most_recent:,
                                   :], x_[-most_recent:, :], sentences[0,
                                   -most_recent:]
    ...
    with plt.style.context('ggplot'):
        ...
        ax.plot(
            ...
            c='k',
            alpha=.5)
        scatter = ax.scatter(list(itertools.chain(*
            [[d] * min(len(data[d]), model.max_sentence_length) for d in dates]
            )), list(itertools.chain(*[list(range(0, -min(len(data[d]),
                model.max_sentence_length), - 1)) for d in dates])),
                c=tokens[tokens > 0.], cmap=cmap, marker='s', s=100)
        del model
