...
class T_HAN(object):
    def __init__(self, mode, **kwargs):
        ...
        Uses an embedding layer, followed by a token-level transformer encoder
        with attention, a sentence-level time-aware-lstm with attention and
        sofrmax layer
        ...
        dropout_keep_prob: float
            percentage of neurons to keep from dropout regularization each
            layer
        ...
        if self.mode == "train":
            ...
            with self.graph.as_default():
                ...
                (
                    ...
                    encoder_class.encoder_multiple_layers()
        ...
        try:
            for epoch in range(curr_epoch, num_epochs):
                ...
                for start, end in zip(
                    range(0, training_size, self.batch_size),
                    range(self.batch_size, training_size, self.batch_size),
                ):
                ...
                # write model into disk at the end of each 10 epoch
                # if epoch > 9 and epoch % 10 == 9:
                saver.save(
                    self.sess,
    ...
    def deploy(self, t_test, x_test):
        ...
        if fake_samples > number_examples:
            ...
            t_test, x_test = (
                t_test[: self.batch_size],
                x_test[: self.batch_size],
            )

        y_probs = np.empty((0))
        for start, end in zip(
            range(0, number_examples + fake_samples + 1, self.batch_size),
            range(
                self.batch_size,
    ...
    def _attention_token_level(self, hidden_state):
        ...
        token_hidden_representation = tf.reshape(
            ...
            shape=[-1, self.max_sentence_length, self.d_model],
        )
    ...
    def _attention_sentence_level(self, hidden_state_sentence):
        ...
        hidden_representation = tf.nn.tanh(
            tf.matmul(hidden_state_2, self.W_w_attention_sentence)
    ...
    def _do_eval(self, eval_x, eval_t, eval_y, writer_val):
        ...
        eval_x, eval_t, eval_y = (
            eval_x[: number_examples - fake_samples],
        ...
        for start, end in zip(
            range(0, number_examples + 1, self.batch_size),
