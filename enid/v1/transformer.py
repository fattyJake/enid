...
class MultiHeadAttention(object):
    ...
    1.linearly project the queries, keys and values h times(with different,
      learned linear projections to d_k, d_k, d_v dimensions)
    ...
    def _linear(x, units, scope=None):
        ...
        with tf.variable_scope(scope or "linear"):
            layer = tf.keras.layers.Dense(
                units, activation=tf.nn.relu, kernel_initializer="he_normal"
            )
            return layer(x)
    ...
    def masking(input_, queries=None, keys=None, type_=None):
        ...
        if type_ in ("k", "key", "keys"):
            ...
            # Generate masks
            masks = tf.sign(
                tf.reduce_sum(tf.abs(queries), axis=-1)
            )  # (N, T_q)
    ...
    def multi_head_attention_fn(self, scope="multi_head_attention"):
        ...
        :param h: h times
        :return: result of scaled dot product attention.
            shape:[sequence_length,d_model]
        """
        with tf.variable_scope(scope):
            # 1. linearly project the queries,keys and values h times(with
            # different,learned linear projections to d_k,d_k,d_v dimensions)
            Q_projected = self._linear(
                self.Q, self.d_model, scope="Q"
            )  # [batch,sequence_length,d_model]
            K_s_projected = self._linear(
                self.K_s, self.d_model, scope="K"
            )  # [batch,sequence_length,d_model]
            V_s_projected = self._linear(
                self.V_s, self.d_model, scope="V"
            )  # [batch,sequence_length,d_model]

            # 2. scaled dot product attention for each projected version of
            # Q,K,V
            ...
            # 3. reshape
            dot_product = tf.concat(
                tf.split(dot_product, self.h, axis=0), axis=2
            )

            # 4. linear projection
            output = self._linear(
                dot_product, self.d_model, scope="output"
            )  # [batch,sequence_length,d_model]
    ...
    def scaled_dot_product_attention_batch(
        ...
        :param V_s:values. shape:[batch,sequence_length,d_model]
        :return: result of scaled dot product attention.
            shape:[batch,h,sequence_length,d_k]
        """
        with tf.variable_scope(scope):
            # 1. split Q,K,V
            Q_heads = tf.concat(
            ...
            # 3. add mask
            dot_product = self.masking(
                dot_product, Q_heads, K_heads, type_="key"
            )
            ...
            weights = tf.nn.dropout(
                weights, self.dropout_keep_prob
            )  # [batch*h,sequence_length,sequence_length]
...
class FeedFoward(object):
    ...
    transformations with a ELU activation in between.

    FFN(x) = max(0,xW1+b1)W2+b2

    While the linear transformations are the same across different positions,
    they use different parameters from layer to layer. Another way of
    describing this is as two convolutions with kernel size 1. The
    dimensionality of input and output is d_model= 512, and the inner-layer has
    ...
    def _linear(x, units, activation, scope=None):
        ...
        with tf.variable_scope(scope or "linear"):
            if activation:
                ...
                layer = tf.keras.layers.Dense(
                    units, kernel_initializer="he_normal"
                )
            return layer(x)
    ...
    def feed_forward_fn(self):
        ...
        # Outer layer
        outputs = self._linear(
            outputs, self.d_model, False, scope="outer_layer"
        )
...
class LayerNormResidualConnection(object):
    ...
    def __init__(
        self, x, y, layer_index, dropout_keep_prob=0.9, use_residual_conn=True
    ):
        self.x = x
        self.y = y
        self.layer_index = layer_index
    ...
    def layer_normalization(self, x, scope="layer_normalization"):
        ...
        with tf.variable_scope(scope + "_" + str(self.layer_index)):
            ...
            variance = tf.reduce_mean(
                tf.square(x - mean), axis=-1, keep_dims=True
            )  # [batch_size,sequence_length,1]
            norm_x = (x - mean) * tf.rsqrt(
                variance + 1e-6
            )  # [batch_size,sequence_length,d_model]

            # 2. re-scale normalized input back
            scale = tf.get_variable(
                "layer_norm_scale", [filter_], initializer=tf.ones_initializer
            )  # [filter]
            bias = tf.get_variable(
                "layer_norm_bias", [filter_], initializer=tf.zeros_initializer
            )  # [filter]
            output = (
                norm_x * scale + bias
            )  # [batch_size,sequence_length,d_model]
...
class Encoder(object):
    ...
    def encoder_multiple_layers(self):
        for layer_index in range(self.num_layer):
            self.Q, self.K_s = self.encoder_single_layer(
                self.Q, self.K_s, layer_index
            )
    ...
    def encoder_single_layer(self, Q, K_s, layer_index):
        ...
        the first is multi-head self-attention mechanism; the second is fully
        connected feed-forward network.
        for each sublayer. use LayerNorm(x+Sublayer(x)). input and output of
        last dimension: d_model
        ...
        multi_head_attention_output = self.sub_layer_multi_head_attention(
            layer_index,
            Q,
            K_s,
            mask=self.mask,
        ...
        # [batch_size,sequence_length,d_model]
        multi_head_attention_output = self.sub_layer_layer_norm_residual_connection(
            K_s,
        ...
        feed_forward_output = self.sub_layer_feed_forward(
            multi_head_attention_output, layer_index
        )

        # 2.2 use LayerNorm(x+Sublayer(x)). all dimension=512.
        feed_forward_output = self.sub_layer_layer_norm_residual_connection(
            multi_head_attention_output,
            feed_forward_output,
            "ff_resid",
            layer_index,
    ...
    def sub_layer_multi_head_attention(
        self, layer_index, Q, K_s, mask=None, dropout_keep_prob=None
    ):  # COMMON FUNCTION
        """
        ...
        :param mask: when use mask,illegal connection will be mask as huge big
            negative value.so it's possiblitity will become zero.
        :return: output of multi head attention.
            shape:[batch_size,sequence_length,d_model]
        """
        with tf.variable_scope(
            "encoder_sub_layer_multi_head_attention_" + str(layer_index)
        ):
            # below is to handle attention for encoder and decoder with
            # difference length:
            ...
            sub_layer_multi_head_attention_output = (
                multi_head_attention_class.multi_head_attention_fn()
            )  # [batch_size*sequence_length,d_model]
    ...
    def sub_layer_feed_forward(self, x, layer_index):  # COMMON FUNCTION
        ...
        """
        with tf.variable_scope(
            "encoder_sub_layer_feed_forward_" + str(layer_index)
        ):
    ...
    def sub_layer_layer_norm_residual_connection(
        ...
        """
        with tf.variable_scope(
            "encoder_sub_layer_norm_residual_"
            + str(layer_index)
            + "_"
            + str(scope)
        ):
