...
class T_LSTM(object):
    ...
    def __init__(
        self, max_sequence_length, hidden_size, pretrain_embedding, learning_rate, decay_steps,
        decay_rate, dropout_keep_prob, l2_reg_lambda=0.0, objective='ce', initializer=tf.orthogonal_initializer()):
        ...
        with tf.name_scope("weight"):
            self.Wi = self._init_weights(self.input_size, self.hidden_size, name='Input_Hidden_weight',reg=None)
            self.Ui = self._init_weights(self.hidden_size, self.hidden_size, name='Input_State_weight',reg=None)
            self.bi = self._init_bias(self.hidden_size, name='Input_Hidden_bias')

            self.Wf = self._init_weights(self.input_size, self.hidden_size, name='Forget_Hidden_weight',reg=None)
            self.Uf = self._init_weights(self.hidden_size, self.hidden_size, name='Forget_State_weight',reg=None)
            self.bf = self._init_bias(self.hidden_size, name='Forget_Hidden_bias')

            self.Wog = self._init_weights(self.input_size, self.hidden_size, name='Output_Hidden_weight',reg=None)
            self.Uog = self._init_weights(self.hidden_size, self.hidden_size, name='Output_State_weight',reg=None)
            self.bog = self._init_bias(self.hidden_size, name='Output_Hidden_bias')

            self.Wc = self._init_weights(self.input_size, self.hidden_size, name='Cell_Hidden_weight',reg=None)
            self.Uc = self._init_weights(self.hidden_size, self.hidden_size, name='Cell_State_weight',reg=None)
            self.bc = self._init_bias(self.hidden_size, name='Cell_Hidden_bias')

            self.W_decomp = self._init_weights(self.hidden_size, self.hidden_size, name='Decomposition_Hidden_weight',reg=None)
            self.b_decomp = self._init_bias(self.hidden_size, name='Decomposition_Hidden_bias_enc')

            self.W_final = self._init_weights(self.hidden_size, 1, name='Output_Layer_weight', reg=None)#tf.contrib.layers.l2_regularizer(scale=0.001)
        ...
        with tf.name_scope("output"):
            self.predictions = tf.nn.xw_plus_b(attention_output, self.W_final, self.b_final, name='predictions')
    ...
    def _init_weights(self, input_size, output_dim, name, std=0.1, reg=None):
        return tf.nn.dropout(
            tf.get_variable(name,shape=[input_size, output_dim],initializer=self.initializer, regularizer = reg),
            self.dropout_keep_prob)

    def _init_bias(self, output_dim, name):
        return tf.get_variable(name,shape=[output_dim],initializer=tf.constant_initializer(0.1))
    ...
    def _attention(self, inputs, attention_size, time_major=False, return_alphas=False):
        ...
        output = tf.reduce_mean(inputs * tf.expand_dims(vu, -1), 1, name='attention_output')

        if not return_alphas: return output
        else: return output#, alphas
            ...
            for epoch in range(curr_epoch, num_epochs):
                ...
                for start, end in zip(range(0, training_size, batch_size), range(batch_size, training_size, batch_size)):
                    ...
                    if counter % evaluate_every == 0:
                        ...
                        dev_accu = _do_eval(sess, model, x_dev, t_dev, y_dev, batch_size)
