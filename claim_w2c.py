...
import math
import os
from enid.data_helper import Vectorizer
...
log_dir = 'w2v_logs'
vec = Vectorizer()
vocabulary_size = vec.variable_size

# Step 1: Download the data.
with open('TLSTM_ED','rb') as f:
    T = pickle.load(f)
    X = pickle.load(f)
    y = pickle.load(f)

vocabulary = X.tolist()
vocabulary = [[idx for idx in sequence if idx != vocabulary_size] for sequence in vocabulary]
data = list(itertools.chain.from_iterable(vocabulary))

# Step 2: Build the dictionary and replace rare words with UNK token.
reverse_dictionary = {i: vec.all_variables[i] for i in range(vocabulary_size)}
...
low_dim_embs = tsne.fit_transform(sampled_embeddings)
labels = [reverse_dictionary[i] for i in idx]
...
xx, yy, zz = low_dim_embs[:, 0], low_dim_embs[:, 1], low_dim_embs[:, 2]
xyzn = zip(xx, yy, zz)
